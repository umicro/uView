	...
	export default {
		...
		methods: {
			...
			onTouchMove(event) {
				...
				// Calculate the percentage value based on the touch position within the slider
				const touchPercentage = this.distanceX / this.sliderRect.width;
				let value = this.min + touchPercentage * (this.max - this.min);
			...
			// },
			updateValue(value, drag) {
				// Format the value to remove decimal places
				const formattedValue = this.format(value);

				// Ensure the value stays within the valid range (between min and max)
				const clampedValue = Math.max(this.min, Math.min(formattedValue, this.max));

				// Calculate the percentage value based on the min and max range
				...
				// During drag, disable the transition animation
				if (drag) {
					barStyle.transition = 'none';
				} else {
					// Remove the transition property to enable the CSS transition
					delete barStyle.transition;
				}

				// Emit the input event with the clamped value
				this.$emit('input', clampedValue);

				// Update the component's barStyle
				this.barStyle = barStyle;
			},
			format(value) {
			...
			onClick(event) {
				if (this.disabled) return;
				// Calculate the percentage based on the click position within the slider
				const clickPercentage = (event.detail.x - this.sliderRect.left) / this.sliderRect.width;
				const value = this.min + clickPercentage * (this.max - this.min);
				this.updateValue(value, false);
			}
		}
	};
