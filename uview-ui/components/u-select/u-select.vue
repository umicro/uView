          ...
          <picker-view @change="columnChange" class="u-select__body__picker-view" :value="defaultSelector" @pickstart="pickstart" @pickend="pickend">
            <picker-view-column v-for="(item, index) in columnData" :key="index">
              <template  v-for="(item1, index1) in item">
                  <view class="u-select__body__picker-view__item" v-if="item1">
                    <view class="u-line-1">
                      {{ item1[labelName] }}
                    </view>
                  </view>
 ...
 * @event {Function} loadData 滚动时异步加载数据，返回（cItem:滚动到的数据,index（滚动的是第几列）,columnIndex：Array select多维数组(滚动后的数据位置索引信息)）
      ...
      if(this.mode == 'mutil-column-auto') {
        ...
        for (let i = 0; i < this.columnNum; i++) {
          ...
          if (i == 0) {
            ...
            data[i] = column;
            column = column[this.defaultSelector[i]]?column[this.defaultSelector[i]][this.childName]:column[this.defaultSelector[i]];
          }
        }
   ...
   async columnChange(e) {
      ...
      if(this.mode == 'mutil-column-auto') {
        ...
        for (let i = index + 1; i < this.columnNum; i++) {
          ...
          // 默认是队列的第一个为默认选项
          let cItem = columnData[i - 1]?columnData[i - 1][i - 1 == index ? columnIndex[index] : 0]:{}
          if(cItem && JSON.stringify(cItem) !="{}"){
            if(cItem[childName] && cItem[childName].length !=0){
              this.columnData[i] = cItem[childName];
              this.defaultSelector[i] = 0;
            }else{
              // this.columnData[i] = [];
              // this.columnData[i+1] = [];
              if(this.loadData) {
                let res = await vm.loadData(cItem,index,columnIndex);
                if(res) {
                  vm.$set(vm['columnData'],i,res.children);
                  vm.defaultSelector[i] = 0;
                }
              }
            }
            // this.$set(this,'columnData',columnData);
            // 改变的列之后的所有列，默认选中第一个
        ...
        columnIndex.map((item, index) => {
          let data = this.columnData[index]?this.columnData[index][columnIndex[index]]:null;
          let tmp = {
